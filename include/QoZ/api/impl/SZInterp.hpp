    ...
    if (!conf.wavelet){
       ...
       // memcpy(cmpDataFirst,cmpData,first);
        ...
        if(conf.transformation==1){
         ...
         QoZ::writefile<T>("waved.qoz.dec.coeff", decData, conf.num);
        QoZ::Wavelet<T,N> wlt;
...
char *SZ_compress_Interp_lorenzo(QoZ::Config &conf, T *data, size_t &outSize) {
    ...
    if(conf.wavelet){
        ...
        QoZ::writefile<T>("waved.qoz.ori.coeff", data, conf.num);
...
//            printf("Lorenzo, quant_bin=8192, ratio = %.2f\n", ratio);
    ...
    if(conf.wavelet){
        ...
        QoZ::writefile<T>("waved.qoz.cmp.coeff", decData, conf.num);
