...
char *SZ_compress_OMP(QoZ::Config &conf, T *data, size_t &outSize) {
...
#pragma omp single
        ...
        if (conf.errorBoundMode == QoZ::EB_REL) {
...
#pragma omp single
        {
            ...
            QoZ::write(nThreads, compressed_pos);
            for (int i = 0; i < nThreads; i++) {
                conf_t[i].save(compressed_pos);
            }
            QoZ::write(cmp_size_t.data(), nThreads, compressed_pos);
        }
